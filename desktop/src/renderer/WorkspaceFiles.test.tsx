...
describe("WorkspaceFileTree", () => {
  ...
  it("opens Markdown files in reading mode by default", async () => {
    ...
    expect(container.querySelector(".workspace-monaco-editor")).toBeNull();
    expect(container.querySelector(".workspace-file-editor-toolbar")).not.toBeNull();
    expect(container.querySelector('[role="tab"][aria-selected="true"]')?.textContent).toContain("阅读");
    expect(writeWorkspaceFile).not.toHaveBeenCalled();
  });

  it("edits and safely saves Markdown source", async () => {
    const originalFile = workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Project Notes\n",
    });
    readWorkspaceFile.mockResolvedValueOnce(originalFile);
    ...
    expect(onDirtyChange).toHaveBeenLastCalledWith({ root: "/repo", path: "README.md", dirty: true });
    ...
    expect(onDirtyChange).toHaveBeenLastCalledWith({ root: "/repo", path: "README.md", dirty: false });
  });

  it("keeps newer Markdown edits made while a save is pending", async () => {
    readWorkspaceFile.mockResolvedValueOnce(workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Original\n",
    ...
    writeWorkspaceFile.mockReturnValueOnce(new Promise((resolve) => {
      finishSave = resolve;
    }));
    const onDirtyChange = vi.fn();
    ...
    await render(
      <WorkspaceFilePreview
        activeContext={activeContext}
        selectedFilePath="/repo/README.md"
        selection={{ startLineNumber: 1, startColumn: 1 }}
        onOpenRightPanel={() => {}}
        onDirtyChange={onDirtyChange}
      />,
    );
    ...
    await act(async () => {
      container.querySelector<HTMLButtonElement>(".mock-editor-save")?.click();
    });
    await act(async () => {
      container.querySelector<HTMLButtonElement>(".mock-editor-edit-second")?.click();
      finishSave?.({
        status: "saved",
        file: workspaceFile({
          path: "README.md",
  ...
  it("keeps a Markdown draft when saving detects an external change", async () => {
    readWorkspaceFile.mockResolvedValueOnce(workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Original\n",
    ...
    expect(container.querySelector("[role=alert]")?.textContent).toContain("文件已在外部发生修改");
    expect(container.querySelector<HTMLElement>(".workspace-monaco-editor")?.dataset.text).toBe("edited code\n");

    await act(async () => {
      container.querySelector<HTMLButtonElement>(".workspace-file-reload-button")?.click();
    });
    expect(container.querySelector<HTMLElement>(".workspace-monaco-editor")?.dataset.text).toBe("# Changed elsewhere\n");
  });

  it("does not replace a dirty Markdown draft during an external refresh", async () => {
    readWorkspaceFile.mockResolvedValueOnce(workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Original\n",
  ...
  it("does not report a conflict when a dirty Markdown file refreshes without content changes", async () => {
    const originalFile = workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Original\n",
    ...
    readWorkspaceFile.mockResolvedValueOnce({ ...originalFile, mtime_ms: 2000 });
    await render(
      <WorkspaceFilePreview
        activeContext={activeContext}
        selectedFilePath="/repo/README.md"
        selection={{ startLineNumber: 1, startColumn: 1 }}
        refreshKey="metadata-refresh"
        onOpenRightPanel={() => {}}
      />,
    );
    await settleDirectoryLoads();
  ...
  it("switches an existing Markdown tab to source mode when a line target arrives", async () => {
    readWorkspaceFile.mockResolvedValueOnce(workspaceFile({
      path: "README.md",
      absolute_path: "/repo/README.md",
      text: "# Project Notes\n\n- review changes\n",
    ...
    expect(container.querySelector<HTMLElement>(".workspace-monaco-editor")?.dataset.selection).toBe("3:1");
  });
