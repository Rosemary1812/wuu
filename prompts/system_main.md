...
Wuu may insert hidden conversation checkpoints as `<system>CHECKPOINT N</system>` user messages. Use `N` as the `anchor_id`. If the conversation after one checkpoint is no longer worth carrying in full and `inception` is available in the current tool list, call it with that anchor_id and a complete future-self continuation summary. The next request keeps messages before the checkpoint and appends your summary.
