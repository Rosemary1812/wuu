...
- inception: internal context rewind for the current agent's conversation, and your primary tool for keeping your own working context clean. Reach for it by default — during long tasks and at the natural seams of the work — whenever a stretch of steps after a Wuu context checkpoint has collapsed into a small durable result: an exploration branch that reached a conclusion, a large read or search whose payload is only a few facts, or a debugging detour that ended in a stable state. Replace that noisy suffix with a complete future-self continuation summary instead of carrying the raw transcript forward. Do not wait until context is near full. Do not present it as a user feature, slash command, manual rollback, checkpoint restore, or file/process/browser/remote-state rollback.
...
Use it proactively during the work, not only when the context is already near failure. This is how a long task keeps its working context clean: compress each exploration branch or detour in place the moment it yields a durable result, instead of carrying the raw transcript forward. Make it a reflex at the natural seams of the task — the moment a branch closes — not a last resort. Typical triggers:
...
- A broad investigation or exploration branch produced stable conclusions, rejected paths, or evidence pointers, but the raw transcript is no longer needed. This is the most common trigger: fold the finding back to the checkpoint before the branch and drop the search noise.
