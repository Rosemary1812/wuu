...
- inception: internal D-Mail-style context rewind for the main agent only. Use it during long tasks when recent steps after a Wuu context checkpoint produced a small stable result from a much larger noisy suffix, and a complete future-self continuation summary can replace that suffix. Do not present it as a user feature, slash command, manual rollback, checkpoint restore, or file/process/browser/remote-state rollback.
