...
## 6.5 设计原则（2026-07-07，用户认定；后续所有「要不要加机制」以此为裁）

1. **数据库管事实，提示词管行为，人管拍板。** 看板/回执类结构负责记录并
   展示发生了什么，不猜意图；提示词引导 agent 行为，但它管不死模型；终审
   永远在用户。让结构猜意图（如自动判定「这句话是不是在派活」）或让提示词
   保证正确性（堆规则堵漏洞）都是越界。
2. **出问题不可怕，看不见才可怕。** agent 的错拦不完；要保证的是出错时
   用户扫一眼就能发现、说一句话就能救。评判一个功能：它是否把「必须盯着」
   变成「扫一眼就知道」。
3. **同一问题真实发生过不止一次、且人工恢复成本高，才值得机制化。**
   出过一次、一句话能救的：记录，不动手。
...
一句话可救）→ 仅把 §3.5 既有的「kickoff 后建任务」句改重，不新增条款；
想象汇报（三次以上、用户会信假话）→ 新增一条核实义务（见 §3.5 修订）；
记账挤占执行 → 观察项，不动。
...
## 8. lead 编排重设计（2026-07-07 定稿，用户逐点裁定）

### 8.1 起因：扁平认领撑不起有依赖的活

2026-07-07 实弹（trace 20260707-212126）：用户 @Andy 让他组队做移动推送
（M2）。Andy 建了**四个 task = 四条孤立 thread**，Han/Mia/Vera 几乎同时
认领各自开跑。铁证——Vera（M2-4 真机验证）的推理原文「gated on M2-1/2/3
landing」「Waiting for M2-1/2/3 to land before starting」：**她自己知道有
依赖，但系统不给她任何表达方式，只能认领了干等，上游落地后也没有信号
叫醒她**。这不是 fabrication（Han/Mia 的 update_status 结论都是实打实的
技术内容），是扁平并行认领模型撑不起「有先后」的活。

对照：作者本人用外部串行驱动跑 MiniMax 军团 7 单从不卡住——因为**编排
在驱动者（人/脚本）手里，不在 wuu 的任务机制里**。唯一一次让军团自组织
（DM 任务）就三轮空转。结论：**任务板在有外部编排者时能跑，要它自己编排
有序的活就停摆。**
...
- **一件多步的活 = 一个 task = 一条 thread = 一张卡 = 一支队。** 卡上挂
  队员头像 + 内部小进度。不再是「四个 task 四条孤立线程」（那正是 Vera
...
- 人只在**头**（群里 @Andy 说要做什么，他不清楚会反问）和**尾**（Andy
  在群里 @用户汇报一句）。中间队伍在 task thread 里干，人不看；想看能翻。
- 计划「发出即跑」——不等用户回 go；用户在头部在场，拆错了当场喊停。
- 真出事不闷着，顶到用户面前（原则 2：人不在回路，更要保证坏消息自己
  找上门）。
- **产品人格 vs 执行模型拆开**：用户对话的是具名 agent（产品层的人）；底下
  lead 可用强模型（authoring 质量要求高），队员可用便宜模型（MiniMax）。

### 8.5 残留风险 + 分期

- **风险变形，未消失**：「lead 中途走神」没了（他不在控制回路）；变成
  「lead 一开始可能把计划拆错（漏依赖/分错人）」——但这在头部可见、可当场
  拦，比看不见的中途走神好治。
- **分期**：本期先落 §8.3 地基规则（a2a 必须 @ + 提示词）+ exec 观测工具，
  用真实军团验证地基。§8.2 的「结构化计划声明 + 系统确定性执行 + 自动
  @交接 + 验证阶段 + 失败再入」是下一增量的主体工程。
