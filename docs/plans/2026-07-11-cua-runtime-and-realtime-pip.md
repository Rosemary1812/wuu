...
8. PiP 属于 session 消息流，只显示当前 session 的 CUA；切换 session 不停止后台 Activity。
...
- PiP 与 Activity 的 `thread_id` 绑定。主界面只显示当前 active session 的 PiP。
- 切换到另一个有活跃 CUA 的 session 时，隐藏旧 PiP并显示新 session 的 PiP。
- 新 session 没有活跃 CUA 时不显示 PiP；切回旧 session 后恢复原 PiP 的位置、尺寸和控制状态。
- 非当前 session 的控制与捕获可以继续运行，但不得占据当前 session 的 UI。
