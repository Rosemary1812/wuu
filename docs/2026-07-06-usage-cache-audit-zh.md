...
即冻结是好的卫生习惯，但对命中率的影响接近零。**真正的 44% 命中率成因当日晚些时候已定位**（对照实验：背靠背新线程前缀哈希字节一致仍零命中；裸请求 r3 同 system + 不同 user 消息 → `cr=128`）：**MiniMax 兼容端点的缓存只做全 prompt 精确前缀匹配，不实现 Anthropic 的 cache_control 断点/最长前缀语义**。线程内（turn 内逐步 91-95%、跨 turn 97%）因请求是严格追加式前缀扩展而命中极高；每开一个新线程首个请求必然全量冷写（user 消息不同即全 miss），会话里新线程越多、聚合命中率被拉得越低——44% 即此。wuu 侧无可修（前缀已字节稳定），属服务端语义限制；产品上的含义是"在 MiniMax 上开新线程的成本 ≈ 一次全量 prompt 写入"。
