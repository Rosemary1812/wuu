...
即冻结是好的卫生习惯，但对命中率的影响接近零。**真正的 44% 命中率成因仍然在逃**。复盘 07-05 trace 时观察到一个更可疑的现象：同一天内 system_hash 完全相同的两个新线程，后开的线程 step0 的 `cache_read` 仅约 128（几乎全量重写缓存），说明**跨线程的前缀复用没有发生**。可能的方向：线程创建间隔超过 TTL、端点按连接/会话分区缓存、或请求头差异。建议后续用两个背靠背的新线程做对照实验定位。
