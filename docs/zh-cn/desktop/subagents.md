...
处理范围较大的任务时，主 Agent 不一定把所有调查、实现和验证都塞进同一段对话。它可以
把边界清楚的一部分工作交给子代理（Subagent），自己继续协调其余工作，最后再汇总和检查
结果。

这不是需要你手动维护的“Agent 团队”。通常只要说明目标和约束，主 Agent 会判断是否值得
拆分；简单或紧密耦合的任务仍由它直接完成。
...
- **子代理**拥有独立的模型上下文，专注一个有明确范围和交付物的任务，例如查找代码、
  研究外部资料、实现独立模块或复核结果。
- **主 Agent 负责整合。** 子代理完成不等于整个任务完成。主 Agent 仍需检查证据、处理冲突、
  运行必要验证，再向你报告结果。
...
子代理。子代理在后台运行时，停止主 Agent 当前一次回复不应被理解为所有子任务都已完成。
...
子代理不是定时任务，也不是云端常驻服务；后台命令同样不会替你理解任务或检查结果。
...
- 当前桌面体验以“主 Agent 自动判断是否委派”为主，暂不提供通过 `@agent` 手动点名子代理
  或切换主 Agent 的交互。
- 子代理适合有边界的临时任务，不代表创建了常驻身份或长期自治团队。
- Wuu 关闭、运行时重启或机器离线时，不能假设正在执行的子代理会像云端服务一样持续运行。
