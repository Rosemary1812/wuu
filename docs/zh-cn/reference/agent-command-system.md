...
> 调查基线：2026-07-22。本文只描述当前已经实现的行为；仍在设计中的生命周期调整不属于公开使用指南。
...
app-server 重启后，新 manager 可以重新观察和停止进程，但无法恢复原来的终端、stdin 或 Go 进程句柄，也可能无法取得可靠的退出码。跨重启任务不提供重启策略、健康检查或退避机制。

正常关闭 app-server 会清理“随会话结束”的任务。删除 thread、关闭 subagent 或从缓存移除 thread 时，它们启动的任务可能继续运行；后台进程也不会让 Desktop 持续保活 app-server。
