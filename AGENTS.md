...
- Desktop macOS release artifacts must be built in CI on a macOS runner, signed with Developer ID, notarized, and uploaded to the same GitHub Release as the GoReleaser CLI assets.
- Required GitHub secrets for desktop release are `MAC_CSC_LINK` and `MAC_CSC_KEY_PASSWORD`, plus either `APPLE_API_KEY`/`APPLE_API_KEY_ID`/`APPLE_API_ISSUER` or `APPLE_ID`/`APPLE_APP_SPECIFIC_PASSWORD`/`APPLE_TEAM_ID`.
- Do not describe a locally built unsigned desktop DMG/ZIP as a public-ready release. Local desktop artifacts are only smoke-test artifacts unless `codesign`, `spctl`, and the installer/archive checks pass.
- For desktop release readiness, verify the same product gates the workflow enforces: `cd desktop && npm test`, production debug controls hidden, `npm run release:desktop` from a clean source state or CI, packaged core version has no `dirty` marker, `codesign --verify`, `spctl --assess`, `hdiutil verify`, and `unzip -t`.
