...
func TestAuthStore_MigratesLegacyAuth(t *testing.T) {
	t.Setenv("WUU_HOME", "")
	home := t.TempDir()
	t.Setenv("HOME", home)

	legacyDir := filepath.Join(home, ".config", "wuu")
	if err := os.MkdirAll(legacyDir, 0o755); err != nil {
		t.Fatalf("mkdir legacy auth dir: %v", err)
	}
	legacyAuth := filepath.Join(legacyDir, "auth.json")
	legacyContents := `{"keys":{"openai":"sk-legacy-auth"}}` + "\n"
	if err := os.WriteFile(legacyAuth, []byte(legacyContents), 0o600); err != nil {
		t.Fatalf("write legacy auth: %v", err)
	}

	key, err := LoadAuthKey(home, "openai")
	if err != nil {
		t.Fatalf("LoadAuthKey after migration: %v", err)
	}
	if key != "sk-legacy-auth" {
		t.Fatalf("migrated auth key = %q, want sk-legacy-auth", key)
	}

	newAuth := filepath.Join(home, ".wuu", "auth.json")
	info, err := os.Stat(newAuth)
	if err != nil {
		t.Fatalf("expected migrated auth at %q: %v", newAuth, err)
	}
	if info.Mode().Perm() != 0o600 {
		t.Fatalf("migrated auth perms = %o, want 0600", info.Mode().Perm())
	}
	if _, err := os.Stat(legacyAuth); err != nil {
		t.Fatalf("legacy auth must be preserved: %v", err)
	}
}

func TestAuthStore_WriteMergesMigratedLegacyKeys(t *testing.T) {
	t.Setenv("WUU_HOME", "")
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	legacyAuth := filepath.Join(legacyDir, "auth.json")
	if err := os.WriteFile(legacyAuth, []byte(`{"keys":{"openai":"sk-legacy"}}`), 0o600); err != nil {
		t.Fatalf("write legacy auth: %v", err)
	}

	// A write should migrate the legacy store first, then add the new key so
	// existing credentials survive.
	if err := SaveAuthKey(home, "anthropic", "sk-new"); err != nil {
		t.Fatalf("SaveAuthKey: %v", err)
	}

	legacy, err := LoadAuthKey(home, "openai")
	if err != nil {
		t.Fatalf("LoadAuthKey legacy: %v", err)
	}
	if legacy != "sk-legacy" {
		t.Fatalf("legacy key lost after write: %q", legacy)
	}
	added, err := LoadAuthKey(home, "anthropic")
	if err != nil {
		t.Fatalf("LoadAuthKey added: %v", err)
	}
	if added != "sk-new" {
		t.Fatalf("new key = %q, want sk-new", added)
	}
}
