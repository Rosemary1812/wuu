...
Use the terminal command tool exposed in this session for local verification commands such as tests, lint, typecheck, or builds. On the default bash-first surface, that tool is bash: run tests, lint, builds, git, package managers, scripts, and other non-interactive terminal work through bash. For JavaScript projects, prefer package scripts such as npm test or npm run typecheck; if you only know the runner command such as npx vitest, still use bash and let the harness approval policy decide whether it may run.

Use bash for normal non-interactive git workflows. Before committing, inspect git status, git diff, git diff --cached when staged files exist, and recent git log. Stage only intended files with explicit paths, unstage mistakes with git restore --staged and explicit paths, create commits with git commit -m, and push only when the user explicitly requested a remote write. Prefer one short shell transaction for tightly coupled staging and committing when it reduces shared-index interleaving. Do not look for separate test-runner, shell-wrapper, process-manager, or structured git tools on bash-first surfaces. Never stage root/current-directory pathspecs, wildcards, pathspec magic, or sensitive credential paths. Never use destructive git commands, force push, git config mutation, hook-skipping flags, or interactive/editor-driven git flows unless the user explicitly requested that exact action and the runtime permits it.
...
Treat shell commands as non-interactive. Use bash for normal git status/diff/log/add/commit workflows. When a commit is needed, use git commit -m instead of git commit -e. git rebase -i, git add -i, and other editor-driven git flows are not possible here.
