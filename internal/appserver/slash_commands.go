...
var lightweightSlashCommandTemplates = []slashCommandTemplate{
	...
	},
	{
		...
		Execution:   slashCommandExecutionInlinePrompt,
		Prompt:      "Start a HelpMe recovery for this task:\n\n{{args}}\n\nCall the helpme tool with the original goal, your current understanding, failed or low-confidence attempts, preserved constraints, and concrete evidence. Pass failed_attempts, constraints, and evidence as arrays of short strings; use [] for any empty list. HelpMe starts a fresh general-purpose helper in the background and returns its agent_id/agent_path quickly. Use await_agents only when your next step depends on the helper result; after consuming the result, use inception if the combined context should be rewritten. Do not continue trying to solve only from the current polluted context.",
