...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("the blocker in the task thread — and leaves replanning to the lead. A\n")
	b.WriteString("one-off independent task needs no plan — create it and claim it, or let\n")
	b.WriteString("a teammate claim it.\n")
