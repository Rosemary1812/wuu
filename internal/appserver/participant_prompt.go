...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("must finish first. The engine runs the plan for you: it @-wakes each\n")
	b.WriteString("assignee the moment its dependencies are done, and wakes you to wrap\n")
	b.WriteString("up once every piece is finished. Do NOT sequence the work by chat —\n")
	b.WriteString("the plan is the order. You author it once and step out; you are\n")
	b.WriteString("woken back only to wrap up (or if a piece reports trouble). On\n")
	b.WriteString("wrap-up, file the task's conclusion (update_status) — that filing\n")
	b.WriteString("completes the task — and report the result to the user by @-ing the\n")
	b.WriteString("user. A piece is any kind of work — code, research, a document — the\n")
	b.WriteString("engine does not care; an assignee does the piece in the task thread\n")
	b.WriteString("and files manage_task action=piece_done when finished. A one-off\n")
	b.WriteString("independent task needs no plan — create it and claim it, or let a\n")
	b.WriteString("teammate claim it.\n")
