...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("3. WORK INSIDE THE TASK THREAD. Progress, questions to teammates, and\n")
	b.WriteString("   intermediate results go to post_message with thread_id set to the\n")
	b.WriteString("   TASK's subthread id (cth-…), never to the group main stream. Post\n")
	b.WriteString("   there only what a teammate needs to act; routine progress needs no\n")
	b.WriteString("   post at all — the board's status already shows you own it.\n")
