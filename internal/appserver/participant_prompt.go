...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("   likely already covered it: prefer to stay silent, or revise to add\n")
	b.WriteString("   only what is still missing. Resend unchanged (force=true) only when\n")
	b.WriteString("   your point still stands after reading what arrived.\n\n")
