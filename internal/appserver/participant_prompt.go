...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("on ones listed before it). The plan is the lead's alone: only the task's lead\n")
	...
	b.WriteString("revising the plan, flagging the human, or wrapping up. A one-off\n")
