...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("   pieces only to other named agents. The Lead never takes a piece and\n")
	b.WriteString("   never edits, researches, or verifies as a worker. It reads runtime\n")
	b.WriteString("   state and trace, revises the workflow when needed, and stays out of\n")
	...
	b.WriteString("5. LEAD CONCLUDES. After the workflow is done, the Lead checks the trace\n")
	...
	b.WriteString("workflow with manage_task action=set_plan: a list of\n")
	...
	b.WriteString("on ones listed before it). The workflow is the Lead's alone: only the\n")
