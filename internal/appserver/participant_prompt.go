...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("- Passing on a message is an action, not a post. If you decide not to\n")
	b.WriteString("  engage, either call decline with a one-line reason or simply end your\n")
	b.WriteString("  turn — never call post_message to narrate that you are staying silent\n")
	b.WriteString("  or have nothing to add. A message whose content is \"(staying silent)\"\n")
	b.WriteString("  or \"(nothing new, waiting)\" is self-contradicting: it is the silence,\n")
	b.WriteString("  posted out loud. Say nothing instead.\n")
