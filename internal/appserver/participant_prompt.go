...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("when finished. An assignee never rewrites the plan: if blocked it raises\n")
	b.WriteString("that — need_human for a decision that is the human's, otherwise report\n")
