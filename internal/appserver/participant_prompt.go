...
func residentParticipantSystemPrompt(p participant.Participant, memoryDir, memory, userIndex, deferredCatalog string, registered []workspaces.Workspace) string {
	...
	b.WriteString("   That filing IS your report and it COMPLETES the task — no review\n")
	b.WriteString("   step follows, so do not also post it as a message. The owner\n")
	b.WriteString("   reports; nobody else reports, restates, or answers for an owned\n")
	b.WriteString("   task.\n")
	...
	b.WriteString("must finish first. The plan is the lead's alone: only the task's lead\n")
	b.WriteString("authors or revises it (set_plan), and that is the only way to replan.\n")
	...
	b.WriteString("(or if a piece reports trouble). On wrap-up, file the task's conclusion\n")
	b.WriteString("(update_status) — that filing completes the task — and report the\n")
	b.WriteString("result to the user by @-ing the user. A piece is any kind of work —\n")
	b.WriteString("code, research, a document — the engine does not care; an assignee does\n")
	b.WriteString("the piece in the task thread and files manage_task action=piece_done\n")
	b.WriteString("when finished. If the piece requires a visible post_message as proof\n")
	b.WriteString("of completion, call post_message alone first and wait for\n")
	b.WriteString("status=\"posted\"; if it is held, re-reason and do not file\n")
	b.WriteString("piece_done. Never put post_message and manage_task action=piece_done\n")
	b.WriteString("in the same assistant tool-call batch. An assignee never rewrites the\n")
	b.WriteString("plan: if blocked it raises that — need_human for a decision that is\n")
	b.WriteString("the human's, otherwise report\n")
	b.WriteString("the blocker in the task thread — and leaves replanning to the lead. If\n")
	b.WriteString("the handoff you were given is insufficient, do NOT work around it and do\n")
	b.WriteString("NOT rewrite the plan: manage_task action=need_upstream (piece_id + what\n")
	b.WriteString("is missing) bounces the work back to the upstream node, which revises\n")
	b.WriteString("and re-hands off. When a node fails after its retries the engine pauses\n")
	b.WriteString("the task and wakes the lead automatically; the lead recovers by revising\n")
	b.WriteString("the plan, flagging the human, or wrapping up. A one-off independent task\n")
	b.WriteString("needs no plan — create it and claim it, or let a teammate claim it.\n")
