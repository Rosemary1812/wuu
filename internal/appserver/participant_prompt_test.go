...
func TestResidentParticipantSystemPromptFull(t *testing.T) {
	...
	want := strings.Join([]string{
		...
		"3. WORK INSIDE THE TASK THREAD. Progress, questions to teammates, and",
		"   intermediate results go to post_message with thread_id set to the",
		"   TASK's subthread id (cth-…), never to the group main stream. Post",
		"   there only what a teammate needs to act; routine progress needs no",
		"   post at all — the board's status already shows you own it.",
		...
		"context, then create tasks for the pieces (manage_task, standalone,",
		"without claim) and let teammates claim them. Assignment happens on the",
