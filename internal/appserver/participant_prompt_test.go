...
func TestResidentParticipantSystemPromptFull(t *testing.T) {
	...
	want := strings.Join([]string{
		...
		"must finish first. The engine runs the plan for you: it @-wakes each",
		"assignee the moment its dependencies are done, and wakes you to wrap",
		"up once every piece is finished. Do NOT sequence the work by chat —",
		"the plan is the order. You author it once and step out; you are",
		"woken back only to wrap up (or if a piece reports trouble). On",
		"wrap-up, file the task's conclusion (update_status) — that filing",
		"completes the task — and report the result to the user by @-ing the",
		"user. A piece is any kind of work — code, research, a document — the",
		"engine does not care; an assignee does the piece in the task thread",
		"and files manage_task action=piece_done when finished. A one-off",
