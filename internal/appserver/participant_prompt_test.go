...
func TestResidentParticipantSystemPromptFull(t *testing.T) {
	...
	want := strings.Join([]string{
		...
		"must finish first. The plan is the lead's alone: only the task's lead",
		...
		"the piece in the task thread and files manage_task action=piece_done",
		"when finished. If the piece requires a visible post_message as proof",
		"of completion, call post_message alone first and wait for",
		"status=\"posted\"; if it is held, re-reason and do not file",
		"piece_done. Never put post_message and manage_task action=piece_done",
		"in the same assistant tool-call batch. An assignee never rewrites the",
		"plan: if blocked it raises that — need_human for a decision that is",
		"the human's, otherwise report",
		"the blocker in the task thread — and leaves replanning to the lead. If",
		"the handoff you were given is insufficient, do NOT work around it and do",
		"NOT rewrite the plan: manage_task action=need_upstream (piece_id + what",
		"is missing) bounces the work back to the upstream node, which revises",
		"and re-hands off. When a node fails after its retries the engine pauses",
		"the task and wakes the lead automatically; the lead recovers by revising",
		"the plan, flagging the human, or wrapping up. A one-off independent task",
		"needs no plan — create it and claim it, or let a teammate claim it.",
