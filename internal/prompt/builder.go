...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("- Call `start_goal` when the user-visible objective needs an active runtime Goal that can keep pushing after an individual model turn stops, or when it is broader than one workflow run or child task, such as a program of work with multiple workflows, approvals, retries, or later resumption. Pass the returned `goal_id` and `goal_dir` to `start_workflow` and workflow-bound `spawn_agent` calls.\n")
	sb.WriteString("- Use `update_goal` for meaningful progress and blockers. The model may report a blocker, but pause, cancel, edit, and limit states are user/system owned; the runtime only marks `blocked` after the same blocker repeats enough times.\n")
	sb.WriteString("- A completed workflow is evidence for a broader Goal, not automatic completion of that Goal. Use `goal_status` before `complete_goal`.\n")
	...
	sb.WriteString("- Use the returned `driver`, tool descriptions, tool result `next_steps`, and `workflow_status` as the source of truth for the exact next action. Use `workflow_control` only after a Workflow Run exists and the run state needs to record planning, worker output, recovery, or final synthesis.\n")
