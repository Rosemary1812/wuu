...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("3. Choose the driver. Use `run_workflow` for executable script-driven workflows; use `create_workflow` when the agent should manage the run directly with workflow tools.\n")
	sb.WriteString("4. After starting a run, use the returned `driver` field (`script` or `agent_managed`) when explaining or deciding who owns phase/spawn/await/synthesis control.\n")
	sb.WriteString("5. In the script driver, prefer `phase(...)`, `spawn(...)` for one worker, `spawnBatch([...])` for a bounded batch of workers, `awaitAgents(...)` for joins, and `synthesize(...)`; the script writes the same Workflow Run, Workflow Team, Agent Run, and final-report state while workers do shell/file work.\n")
	sb.WriteString("6. In an agent-managed run, call `list_agent_profiles`, decide which roles reuse profiles, create profiles, or stay ephemeral, then record the Workflow Team with `workflow_control` action=record_workflow_team.\n")
	sb.WriteString("7. Use `spawn_agent` for agent-managed workflow work. Set `agent_profile` for reuse_profile/create_profile team members; omit it for ephemeral workers.\n")
	sb.WriteString("8. Use `workflow_status` to inspect progress before synthesis, and use `workflow_control` actions like pause_run, resume_run, retry_agent_run, create_file_checkpoint, restore_file_checkpoint, and record_memory_candidate for run recovery.\n")
	sb.WriteString("9. The dynamic Agent instances for a run are temporary; durable memory belongs to named Agent Profiles, while project/global memory remains shared background.\n")
	sb.WriteString("10. Use workflow file checkpoints before risky direct edits and restore_file_checkpoint for scoped rollback.\n")
	sb.WriteString("11. Use `save_workflow` when an ad hoc script or plan should become a reusable project or user workflow definition.\n")
	sb.WriteString("12. For scheduled workflow-shaped tasks, use `schedule_cron` with workflow_name so the scheduled agent prompt can start the Workflow Run.\n")
	sb.WriteString("13. Record long-term memory candidates through `workflow_control`; do not write profile memory directly from ordinary workflow progress.\n\n")
