...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("3. Start the run with `start_workflow` using driver=auto unless the user or workflow explicitly requires a lower-level driver.\n")
	sb.WriteString("4. `start_workflow` dispatches script definitions to `run_workflow` and agent-managed definitions or ad hoc plans to `create_workflow`; call those lower-level tools directly only when you must force a driver.\n")
