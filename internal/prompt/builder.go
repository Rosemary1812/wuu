...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("4. In the script driver, prefer `phase(...)`, `spawn(...)` for one worker, `spawnBatch([...])` for a bounded batch of workers, `awaitAgents(...)` for joins, and `synthesize(...)`; the script writes the same Workflow Run, Workflow Team, Agent Run, and final-report state while workers do shell/file work.\n")
	sb.WriteString("5. In an agent-managed run, call `list_agent_profiles`, decide which roles reuse profiles, create profiles, or stay ephemeral, then record the Workflow Team with `workflow_control` action=record_workflow_team.\n")
	sb.WriteString("6. Use `spawn_agent` for agent-managed workflow work. Set `agent_profile` for reuse_profile/create_profile team members; omit it for ephemeral workers.\n")
	sb.WriteString("7. Use `workflow_status` to inspect progress before synthesis, and use `workflow_control` actions like pause_run, resume_run, retry_agent_run, create_file_checkpoint, restore_file_checkpoint, and record_memory_candidate for run recovery.\n")
