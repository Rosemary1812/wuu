...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("The following workflows are available in this session. A workflow is a reusable, multi-agent coordination process with durable run state. ")
	sb.WriteString("Use workflows for long-running, scheduled, repeatable, or multi-phase work where chat context should not be the only state store. Workflow state is durable, but a background script may still need an active runtime to keep executing.\n\n")
	...
	sb.WriteString("3. For dynamic JavaScript workflow scripts, call `run_workflow`. Inside the script, prefer `phase(...)`, `spawn(...)` for one worker, `spawnBatch([...])` for a bounded batch of workers, `awaitAgents(...)` for joins, and `synthesize(...)`; the script keeps orchestration state in the Workflow Run while workers do shell/file work.\n")
