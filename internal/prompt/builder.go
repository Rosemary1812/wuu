...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("**Goal binding rule:**\n")
	...
	sb.WriteString("- Call `start_goal` first only when the objective is broader than one workflow run or child task, such as a program of work with multiple workflows, approvals, retries, or later resumption. Pass the returned `goal_id` and `goal_dir` to `start_workflow` and workflow-bound `spawn_agent` calls.\n")
	...
	sb.WriteString("3. Start the run with `start_workflow`. The `driver` argument defaults to `auto`, which selects `script` or `agent_managed` from the saved workflow definition, or uses the inline `script` field if you pass one; override with `driver: \"script\"` or `driver: \"agent_managed\"` only when the user, workflow, or recovery path explicitly requires it.\n")
	sb.WriteString("4. `start_workflow` returns the durable Workflow Run id and the chosen `driver` value (`script` or `agent_managed`).\n")
