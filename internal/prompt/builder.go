...
func (b *Builder) AddWorkflows(workflows []workflow.Definition) {
	...
	sb.WriteString("- Call `start_goal` when the user-visible objective needs an active runtime Goal that can keep pushing after an individual model turn stops, or when it is broader than one workflow run or child task, such as a program of work with multiple workflows, approvals, retries, or later resumption. Runtime Goals own continuation; workflow and subagent reports are evidence.\n")
	sb.WriteString("- Use `update_goal` only to report a real blocker on the active runtime Goal. Do not use it for progress notes or decisions. The model may report a blocker, but pause, cancel, edit, and limit states are user/system owned; the runtime only marks `blocked` after the same blocker repeats enough times.\n")
