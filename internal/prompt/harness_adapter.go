...
func HarnessAdapterText(providerName, model string) string {
	...
	case HarnessFamilyClaude:
		return header + strings.Join([]string{
			"- Prefer the dynamic workflow path for executable workflow definitions: load the saved script, call run_workflow, and let the script manage phase/spawn/await/synthesize state.",
	...
	case HarnessFamilyCodex, HarnessFamilyGPT:
		return header + strings.Join([]string{
			...
			"- Use workflow scripts only when a saved .js workflow or explicitly repeatable multi-agent process applies; do not turn ordinary implementation into workflow ceremony.",
	...
	case HarnessFamilyGemini, HarnessFamilyKimi:
		return header + strings.Join([]string{
			...
			"- Prefer portable primitives: spawn_agent for delegated workers, run_workflow for executable workflow scripts, create_workflow for manual workflow state.",
	...
	default:
		return header + strings.Join([]string{
			...
			"- Prefer generic primitives: spawn_agent for delegated workers, run_workflow for executable workflow scripts, create_workflow for manual workflow state.",
