...
- Use bash for normal git status/diff/log/add/commit workflows. For commit flows, inspect status and diff, stage intended files with explicit paths, unstage mistakes with git restore --staged when needed, commit with ` + "`git commit -m`" + `, and push only when explicitly requested. Bash is the terminal entry point for tests, git, scripts, and process work. Never use ` + "`git commit -e`" + `, ` + "`git rebase -i`" + `, ` + "`git add -i`" + `, root/current-directory staging, wildcards, git config mutation, hook-skipping flags, force push, or other editor-driven/destructive git flows unless the user explicitly requested that exact action and the runtime permits it.
