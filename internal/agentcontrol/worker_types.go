...
- Use run_shell for normal git status/diff/log/add/commit workflows. For commit flows, inspect status and diff, stage intended files with explicit paths, unstage mistakes with git restore --staged when needed, commit with ` + "`git commit -m`" + `, and push only when explicitly requested. Use the structured git tool when structured status output or restricted helpers are useful. Never use ` + "`git commit -e`" + `, ` + "`git rebase -i`" + `, ` + "`git add -i`" + `, root/current-directory staging, wildcards, git config mutation, hook-skipping flags, force push, or other editor-driven/destructive git flows unless the user explicitly requested that exact action and the runtime permits it.
...
func readOnlyPlanningTools() []string {
	return []string{"read_file", "grep", "glob", "run_shell", "agent_report"}
}

func readOnlyVerificationTools() []string {
	return []string{"read_file", "grep", "glob", "run_shell", "run_test", "agent_report"}
}
