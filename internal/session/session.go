...
func insertHistoryRecordTx(tx *sql.Tx, id string, seq int, rec HistoryRecord) error {
	_, err := tx.Exec(`
		...
		strings.TrimSpace(rec.Provider), strings.TrimSpace(rec.Model), strings.TrimSpace(rec.ParticipantID),
	)
	if err != nil {
...
WHERE session_id = ?`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&rec.Provider, &rec.Model, &rec.ParticipantID,
		); err != nil {
