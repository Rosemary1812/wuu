...
func TestRewriteHistoryWithInceptionContinuationCutsAfterAnchor(t *testing.T) {
	...
	if rewritten[2].Role != "user" || rewritten[2].Name != ContextContinuationName || !strings.Contains(rewritten[2].Content, InceptionContinuationPrefix) || !strings.Contains(rewritten[2].Content, "external state remain current") {
